...
#Data and Output
packedSimPath		 =	ph.getParam( "packedSimPath",		 '../2ddata_sim/' ) 	# path to training data
...
frame_max		= int(ph.getParam( "frame_max",		   189 ))
...
def generate3DUniForNewNetwork(imageindex = 0, outPath = '../', ):
	...
	if generateUni:
		...
		if upsampling_mode == 2:
			if upsampleFirst:
				np.savez_compressed(packedSimPath + '/sim_%04d/density_low_2x2_%04d.npz'%(fromSim,  imageindex+frame_min),
									dim_output)
			else:
				np.savez_compressed(packedSimPath + '/sim_%04d/density_low_2x2x1_%04d.npz'%(fromSim, imageindex+frame_min),
									dim_output)
		elif upsampling_mode == 1:
			np.savez_compressed(packedSimPath + '/sim_%04d/density_low_1x1_%04d.npz'%(fromSim, imageindex+frame_min),
								dim_output)
		elif upsampling_mode == 0:
			np.savez_compressed(packedSimPath + '/sim_%04d/density_low_1x1x1_%04d.npz'%(fromSim, imageindex+frame_min),
								dim_output)
		elif upsampling_mode == 3:
			np.savez_compressed(packedSimPath + '/sim_%04d/density_low_0x0_%04d.npz'%(fromSim, imageindex+frame_min),
								dim_output)
