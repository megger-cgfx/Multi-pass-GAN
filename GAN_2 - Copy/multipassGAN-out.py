...
basePath		=	 ph.getParam( "basePath",		'../3ddata_gan_sliced/' )
randSeed		= int(ph.getParam( "randSeed",		1 )) 				# seed for np and tf initialization
load_model_test_1 = int(ph.getParam( "load_model_test_1", 4 )) 			# 4 # the number of the test to load a model from. can be used in training and output mode. -1 to not load a model
load_model_test_2 = int(ph.getParam( "load_model_test_2", -1 )) 			# 48 # the number of the test to load a model from. can be used in training and output mode. -1 to not load a model
...
load_model_no_1   = int(ph.getParam( "load_model_no_1",   1199 )) 			# 1199 # nubmber of the model to load
load_model_no_2   = int(ph.getParam( "load_model_no_2",   -1 )) 			# 799 # nubmber of the model to load
...
#Data and Output
packedSimPath		 =	 ph.getParam( "packedSimPath",		 '../3ddata_sim/' ) 	# path to training data
fromSim		= int(ph.getParam( "fromSim",		 1000 )) 			# range of sim data to use, start index
toSim 		= fromSim
frame_min		= int(ph.getParam( "frame_min",		   100 ))
...
generateUni = int(ph.getParam("genUni", True))
...
def generate3DUniForNewNetwork(imageindex = 0, outPath = '../', inputPer = 3.0, head = None):
	...
	if generateUni:
		...
		np.savez_compressed(packedSimPath + '/sim_%04d/source_%04d.npz'%(fromSim, imageindex+frame_min), dim_output)
		print('stored .npz file')
	return		
...
#head_0, _ = uniio.readUni(packedSimPath + "sim_%04d/density_low_%04d.npz"%(fromSim, 0))
